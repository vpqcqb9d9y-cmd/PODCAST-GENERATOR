...
- **Docs**: Added bilingual user guides covering every panel/button (see `docs/USER_GUIDE.md` and `docs/USER_GUIDE.he.md`).

## Hotfix 3.4.3
- **Exact subtitle timing**: SRTs now anchor to real TTS segment durations (per WAV) instead of word-weighted heuristics, removing drift between speakers/engines.
- **Pipeline passthrough**: Runner forwards per-segment durations to stitching/video/subtitle steps to keep captions aligned with stitched audio.
- **Backward-safe**: Falls back to the previous heuristic only when durations are unavailable; lead-in/outro padding respected.
